...
#'@return a `autoenc_conv2d_e` object.
#'@examples
#'#See an example of using `autoenc_conv2d_e` at this
#'#https://github.com/cefet-rj-dal/daltoolbox/blob/main/autoencoder/autoenc_conv2d_e.md
#'@import reticulate
#'@export
autoenc_conv2d_e <- function(input_size, encoding_size, batch_size = 32, num_epochs = 50, learning_rate = 0.001) {
  obj <- dal_transform()
  ...
  class(obj) <- append("autoenc_conv2d_e", class(obj))

  return(obj)
}

#'@export
fit.autoenc_conv2d_e <- function(obj, data, ...) {
...
#'@export
transform.autoenc_conv2d_e <- function(obj, data, ...) {
  if (!exists("autoenc_conv2d_create"))
    reticulate::source_python(system.file("python", "autoenc_conv2d.py", package = "daltoolboxdp"))

  result <- NULL
  if (!is.null(obj$model))
    result <- autoenc_conv2d_encode(obj$model, data)
  return(result)
}
